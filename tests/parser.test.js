 ...
 */
import { DataKind, Endian, FrameDecoder, compileDescriptor, mergeGnssVec8, parseGPGGA, parseGPVTG, parseNovAV } from '../src/shared';
import { byteSizeOf, readField, SCALE_NONE } from '../src/shared/parser/datatype';
function makeField(role, name, type, def = '0', scale = SCALE_NONE) {
    return {
        id: name,
        name,
        type,
...
console.log('\n[8] gnss: GPGGA');
{
    ...
    if (!d)
        throw new Error('GPGGA parse failed');
...
console.log('\n[9] gnss: GPVTG');
{
    ...
    if (!d)
        throw new Error('GPVTG parse failed');
...
console.log('\n[11] gnss: BESTVEL/BESTPOS（NovAtel）');
{
    ...
    if (!d)
        throw new Error('BESTVEL parse failed');
